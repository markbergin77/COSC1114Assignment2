...
#include <unistd.h>
#include <string.h>
#define CENTRAL_MAILBOX1 3539565100    //Central Mailbox number 
#define CENTRAL_MAILBOX2 3539565200
...
int main(int argc, char *argv[]) {
	...
	//Error
   	 if(pid < 0) 
   	 {
   	     fprintf(stderr, "ERROR: Child failed to create\n");
   	 }
	//Child deals with process 5-8
   	 else if(pid == 0) 
   	 {
   	     initTemp = atoi(argv[2]);
   	     if(strcmp(argv[4],"1300") == 0)
	{
		server = CENTRAL_MAILBOX2;
	}
   	     strcpy(processTitle, "[CHILD]");
  	  }
   	 else
		//Parent deals with 1-4
  	  {
   	     initTemp = atoi(argv[1]);
   	     if(strcmp(argv[3],"1200") == 0)
    {
	server = CENTRAL_MAILBOX1;
    }
   	     strcpy(processTitle, "[PARENT]");
   	 }


   	//Create the Central Servers Mailbox
   	int msqidC = msgget(server, 0600 | IPC_CREAT);

   	//Create the mailboxes for the other processes and store their IDs
   	for(i = 1; i <= NUM_PROCESSES; i++){
      		msqid[(i-1)] = msgget((server + i), 0600 | IPC_CREAT);
   	}
