...
#define CENTRAL_MAILBOX 3539565100    /*Central Mailbox number*/
...
int main(int argc, char *argv[]) {
	...
	{
		serverKey = CENTRAL_MAILBOX;
	}
	else if(strcmp(argv[3],"1300") == 0)
	{
		serverKey = CENTRAL_MAILBOX2;
	}
	else
	{
		printf("\nMAILBOX GIVEN DOESN'T EXIST\n");
		exit(0);
	}
   	...
   	int uid = atoi(argv[2]);
        //Making sure the processes start from 1,
        //due to different mailboxes.
	if(uid >  4)
	{
		uid = (uid - 4);
	}
   	/*Create the Central Servers Mailbox*/
   	int msqidC = msgget(serverKey, 0600 | IPC_CREAT);

   	/*Create the mailbox for this process and store it's IDs*/
   	int msqid = msgget((serverKey + uid), 0600 | IPC_CREAT);
